...
public class VincyusCommand implements CommandExecutor
{
    private acapi plugin;

    public VincyusCommand() {
        this.plugin = acpi.getInstance();
    }

    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        if (!cmd.getName().equalsIgnoreCase("vincyus")) {
            return false;
        }
        ...
        if (args.length == 0) {
            if (!p.hasPermission("acapivin.admin")) {
                ...
                this.plugin.sms(p, "&7By: &aVinCyus_");
